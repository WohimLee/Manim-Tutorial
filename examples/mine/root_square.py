...
class SqrtRootScene(Scene):
    def construct(self):
        ...
        # # 2. Goal
        # target1 = Tex(r"We want to find the value of $\sqrt{a}$")
        # target2 = Tex(r"which is the square root of $a$")

        # target1.next_to(title, ORIGIN)
        ...
        step1 = VGroup(assume, equation).arrange(DOWN, aligned_edge=ORIGIN)
        step1.shift(UP)
        self.play(Write(step1))
        self.wait()

        # 移除 assume，保留 equation
        self.play(
            FadeOut(assume, shift=UP)
        )
        self.play(
        ...
        # 显示描述文字
        desc = Text("Square both sides", font_size=35)
        desc.shift(UP*2.5)
        
        # 创建新等式，并放在 equation 下方
        new_equation = Tex(r"$x^2 = a$")
        # new_equation.next_to(equation, DOWN)
        ...
        # 移除旧等式和箭头，保留 new_equation 并移至原 equation 位置
        self.play(
            FadeOut(equation),
            FadeOut(arrow),
            FadeOut(desc),
            new_equation.animate.move_to(equation.get_center())
        )
        self.wait()
